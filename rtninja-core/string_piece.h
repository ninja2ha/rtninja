...
void CopyToString(const WStringPiece& self, std::wstring* target);

void AppendToString(const StringPiece& self, std::string* target);
void AppendToString(const WStringPiece& self, std::wstring* target);

size_t copy(const StringPiece& self,
            char* buf,
            size_t n,
            size_t pos);
size_t copy(const WStringPiece& self,
            wchar_t* buf,
            size_t n,
            size_t pos);
...
size_t find(const StringPiece& self,
            const StringPiece& s,
            size_t pos);
size_t find(const WStringPiece& self,
            const WStringPiece& s,
            size_t pos);
size_t find(const StringPiece& self,
            char c,
            size_t pos);
...
size_t rfind(const StringPiece& self,
             const StringPiece& s,
             size_t pos);
size_t rfind(const WStringPiece& self,
             const WStringPiece& s,
             size_t pos);
size_t rfind(const StringPiece& self,
             char c,
...
size_t find_first_of(const StringPiece& self,
                     const StringPiece& s,
                     size_t pos);
size_t find_first_of(const WStringPiece& self,
                     const WStringPiece& s,
                     size_t pos);
...
size_t find_first_not_of(const StringPiece& self,
                         const StringPiece& s,
                         size_t pos);
size_t find_first_not_of(const WStringPiece& self,
                         const WStringPiece& s,
                         size_t pos);
size_t find_first_not_of(const StringPiece& self,
                         char c,
...
size_t find_last_of(const StringPiece& self,
                    const StringPiece& s,
                    size_t pos);
size_t find_last_of(const WStringPiece& self,
                    const WStringPiece& s,
                    size_t pos);
size_t find_last_of(const StringPiece& self,
                    char c,
...
size_t find_last_not_of(const StringPiece& self,
                        const StringPiece& s,
                        size_t pos);
size_t find_last_not_of(const WStringPiece& self,
                        const WStringPiece& s,
                        size_t pos);
size_t find_last_not_of(const WStringPiece& self,
                          wchar_t c,
...
// StringPiece and WStringPiece. Do not refer to this class directly, but
// rather to BasicStringPiece, StringPiece, or WStringPiece.
//
// This is templatized by string class type rather than character type, so
// BasicStringPiece<std::string> or BasicStringPiece<std::wstring>.
template <typename STRING_TYPE> class BasicStringPiece {
 public:
 ...
 public:
  ...
  // expected (likewise for wchar_t, std::wstring, WStringPiece).
...
// WStringPiece operators -----------------------------------------------------

inline bool operator==(const WStringPiece& x, const WStringPiece& y) {
  if (x.size() != y.size())
    return false;

  return WStringPiece::wordmemcmp(x.data(), y.data(), x.size()) == 0;
}
...
inline bool operator<(const WStringPiece& x, const WStringPiece& y) {
  const int r = WStringPiece::wordmemcmp(
      x.data(), y.data(), (x.size() < y.size() ? x.size() : y.size()));
  return ((r < 0) || ((r == 0) && (x.size() < y.size())));
}
...
// We provide appropriate hash functions so StringPiece and WStringPiece can
// be used as keys in hash sets and maps.

// This hash function is copied from base/containers/hash_tables.h. We don't
// use the ones already defined for string and std::wstring directly because it
// would require the string constructors to be called, which we don't want.
#define RTNINJA_HASH_STRING_PIECE(StringPieceType, string_piece)        \
  std::size_t result = 0;                                               \
...
template<>
struct hash<rtninja::StringPiece> {
  std::size_t operator()(const rtninja::StringPiece& sp) const {
    RTNINJA_HASH_STRING_PIECE(rtninja::StringPiece, sp);
  }
};
template<>
struct hash<rtninja::WStringPiece> {
  std::size_t operator()(const rtninja::WStringPiece& sp16) const {
    RTNINJA_HASH_STRING_PIECE(rtninja::WStringPiece, sp16);
  }
};

}  // namespace std

#endif  // RTNINJA_RTNINJA_CORE_STRING_PIECE_H_